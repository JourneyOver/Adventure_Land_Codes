...
function disconnect() {
    ...
    $("body").append("<div style='position: fixed; top: 0px; left: 0px; right: 0px; bottom: 0px; z-index: 999; background: rgba(0,0,0,0.85); text-align: center'><div onclick='refresh_page()' class='gamebutton clickable' style='margin-top: " + (round(height / 2) - 10) + "px'>DISCONNECTED</div></div>");
    if (socket) {
        socket = null ,
        socket.disconnect()
    }
...
function position_map() {
    ...
    if (character) {
        if (manual_centering) {
            character.x = c_round(width / 2),
            character.y = c_round(height / 2)
        } else {
...
function process_entities(f, d) {
    for (var b = 0; b < f.monsters.length; b++) {
        var c = f.monsters[b];
        var h = entities[c.id];
        if (!h) {
            if (c.dead) {
            ...
            try {
                h = entities[c.id] = add_monster(c)
            } catch (g) {
                console.log("EMAIL HELLO@ADVENTURE.LAND WITH THIS: " + JSON.stringify(c))
            }
            h.drawn = false;
            h.resync = true
        }
        ...
        sync_entity(h, c);
        (c.events || []).forEach(function(j) {
            if (j.type == "mhit") {
                var k = get_entity(j.p)
                  , e = get_entity(j.m);
                if (!k) {
                    return
                }
                ...
                start_animation(k, h.hit || "slash0");
                d_line(h, k);
                if (in_arr(h.hit, ["explode_a", "explode_c"])) {
                    sfx("explosion")
                } else {
                    sfx("monster_hit")
                }
                if (!j.k) {} else {
                    start_animation(k, "spark0")
                }
            }
        })
    }
    for (var b = 0; b < f.players.length; b++) {
        var a = f.players[b];
        var h = entities[a.id];
        ...
        if (!h) {
            ...
            a.external = true;
            a.player = true;
            h = entities[a.id] = add_character(a);
            h.drawn = false;
            h.resync = true
        }
        if (a.dead) {
...
function adopt_soft_properties(a, b) {
    ...
    for (prop in b) {
        if (in_arr(prop, ["x", "y", "vx", "vy", "moving", "abs", "going_x", "going_y", "from_x", "from_y", "width", "height", "type", "events", "angle"])) {
            continue
        }
        a[prop] = b[prop]
    }
...
function update_overlays() {
    ...
    if (character) {
        ...
        if (!cached("coord", round(map.real_x / 10) + "|" + round(map.real_y / 10))) {
            $(".coords").html(round(map.real_x / 10) + "," + round(map.real_y / 10))
        }
        if (!topleft_npc) {
            reset_topleft()
        }
...
function init_socket() {
    ...
    socket.on("start", function(data) {
        ...
        $(".xpsui").show();
        $("body").append('<input id="chatinput" onkeypress="if(event.keyCode==13) socket.emit(\'say\',{message:$(this).rfval()})" placeholder=""/>');
        if (gtest) {
            $("body").children().each(function() {
...
function create_map() {
    ...
    for (var g = 0; g <= M.tiles.length; g++) {
        if (g == M.tiles.length) {
            element = M["default"]
        } else {
            element = M.tiles[g]
        }
        sprite_last[current_map][g] = 0;
        if (!tile_sprites[current_map][g]) {
            tile_sprites[current_map][g] = [];
            if (!element) {
                continue
            }
            if (element[3].length) {
            ...
            var o = new PIXI.Rectangle(element[1],element[2],element[3],element[4]);
            var l = new PIXI.Texture(C[G.tilesets[element[0]]],o);
            element[5] = l;
            if (element[0] == "water") {
                element[5].type = "water";
                o = new PIXI.Rectangle(element[1] + 48,element[2],element[3],element[4]);
                l = new PIXI.Texture(C[G.tilesets[element[0]]],o);
                element[6] = l;
                o = new PIXI.Rectangle(element[1] + 48 + 48,element[2],element[3],element[4]);
                l = new PIXI.Texture(C[G.tilesets[element[0]]],o);
                element[7] = l
            }
            if (element[0] == "puzzle" || element[0] == "custom_a") {
                element[5].type = "water";
                o = new PIXI.Rectangle(element[1] + 16,element[2],element[3],element[4]);
                l = new PIXI.Texture(C[G.tilesets[element[0]]],o);
                element[6] = l;
                o = new PIXI.Rectangle(element[1] + 16 + 16,element[2],element[3],element[4]);
                l = new PIXI.Texture(C[G.tilesets[element[0]]],o);
                element[7] = l
            }
            tile_sprites[current_map][g][sprite_last[current_map][g]] = new_map_tile(element)
        }
    }
    if (M.groups) {
        for (var b = 0; b < M.groups.length; b++) {
            if (!M.groups[b].length) {
                continue
            }
            ...
            var n = 999999999
              , q = 99999999
              , r = -999999999;
            for (var g = 0; g < M.groups[b].length; g++) {
                var j = M.groups[b][g]
                  , d = M.tiles[j[0]];
                if (j[1] < q) {
                    q = j[1]
            ...
            for (var g = 0; g < M.groups[b].length; g++) {
                var j = M.groups[b][g];
                var f = new PIXI.Sprite(M.tiles[j[0]][5]);
                f.x = j[1] - q;
                f.y = j[2] - n;
                if (j[2] < n) {
                    n = j[2]
    ...
    npcs = map_info.npcs;
    for (var g = 0; g < npcs.length; g++) {
        var k = npcs[g]
          , d = G.npcs[k.id];
        if (d.type == "full") {
            continue
        }
        console.log("NPC: " + k.name);
        var m = add_npc(d, k.position, k.name, k.id);
        map.addChild(m);
        map_npcs.push(m)
    }
...
function draw(a, b) {
    ...
    if (!pull_all) {
        entities_data.forEach(function(f) {
            process_entities(f)
        })
    }
    entities_data = [];
