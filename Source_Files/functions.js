...
function trigger_runner(a, b) {
    if (!code_run) {
        return
    }
    if (!(document.getElementById(a) && document.getElementById(a).contentWindow && is_function(document.getElementById(a).contentWindow.render))) {
        setTimeout(function() {
            trigger_runner(a, b)
        }, 300)
    } else {
        document.getElementById(a).contentWindow.render(b, "js");
        socket.emit("code", {
            run: 1
        })
    }
